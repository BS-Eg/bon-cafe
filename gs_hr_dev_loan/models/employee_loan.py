...
class employee_loan(models.Model):
    ...
    def send_loan_detail(self):
        if self.employee_id and self.employee_id.work_email:
            template_id = self.env.ref('gs_hr_dev_loan.dev_employee_loan_detail_send_mail')
    ...
    def get_loan_url(self):
        for loan in self:
            ...
            action_id = self.env.ref('gs_hr_dev_loan.action_employee_loan').id
            menu_id = self.env.ref('gs_hr_dev_loan.menu_employee_loan').id
            if base_url:
    ...
    def action_send_request(self):
        ...
        if self.manager_id and self.manager_id.work_email:
            ...
            template_id = self.env.ref('gs_hr_dev_loan.dev_dep_manager_request')
    ...
    def dep_manager_approval_loan(self):
        ...
        if email:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.dev_hr_manager_request')
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id.id)
    ...
    def hr_manager_approval_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.hr_manager_confirm_loan')
    ...
    def dep_manager_reject_loan(self):
        ...
        if self.employee_id.work_email:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.dep_manager_reject_loan')
    ...
    def action_close_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.hr_manager_closed_loan')
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id.id)
            template_id.write({'email_to': self.employee_id.work_email})
            template_id.send_mail(self.ids[0], True)
    ...
    def hr_manager_reject_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = self.env.ref('gs_hr_dev_loan.hr_manager_reject_loan')
    ...
    def action_view_loan_installment(self):
        action = self.env.ref('gs_hr_dev_loan.action_installment_line').read()[0]

        installment = self.mapped('installment_lines')
        if len(installment) > 1:
            action['domain'] = [('id', 'in', installment.ids)]
        elif installment:
            action['views'] = [(self.env.ref('gs_hr_dev_loan.view_loan_emi_form').id, 'form')]
