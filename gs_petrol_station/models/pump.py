...
class GSPump(models.Model):
    ...
    def compute_sum_liter(self):
        for rec in self:
            ...
            for line in rec.pump_line_ids:
                if line.type_pump_id == self.env.ref("gs_petrol_station.gs_petrol_91"):
                    rec.sum_petrol_91 += line.opening_pump
                elif line.type_pump_id == self.env.ref("gs_petrol_station.gs_petrol_95"):
                    rec.sum_petrol_95 += line.opening_pump
                elif line.type_pump_id == self.env.ref("gs_petrol_station.gs_diesel"):
    ...
    def compute_is_admin(self):
        for rec in self:
            rec.is_admin = False
            if self.env.user.has_group('gs_petrol_station.group_gs_petrol_station_Admin'):
                rec.is_admin = True
