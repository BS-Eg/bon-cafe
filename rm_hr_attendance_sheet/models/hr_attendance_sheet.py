...
class AttendanceSheet(models.Model):
    ...
    def action_draft(self):
        for rec in self:
            if rec.state in ['confirm','done']:
                raise ValidationError(_("You can't Set to draft if sheet confirmed or approved"))
            else:
                rec.write({'state': 'draft'})
    def unlink(self):
        for rec in self:
            if rec.state in ['confirm','done']:
                raise ValidationError(_("You can't delete if sheet confirmed or approved"))
        return super(AttendanceSheet, self).unlink()
                ...
                if work_intervals:
                    if public_holiday:
                        ...
                        for i, work_interval in enumerate(work_intervals):
                            ...
                            rate = att_sheet.contract_id.resource_calendar_id.hours_per_day
                            act_float_late = late_in.total_seconds() / 3600
                            policy_late = policy_id.get_late(rate,float_late,late_cnt)
                            if policy_late > 0.0:
                                late_cnt += 1
                            ...
                            if status == 'ab':
                                ...
                                act_float_diff = float_diff
                                float_diff = policy_id.get_diff(rate,float_diff,diff_cnt)
                                if float_diff > 0.0:
                                    diff_cnt += 1
    ...
    def action_create_payslip(self):
        ...
        for sheet in self:
            ...
            name ='Attendance Sheet - %s - %s' % (sheet.employee_id.name or '',
                                            format_date(self.env,
                                                        sheet.date_from,
                                                        date_format="MMMM y"))
    ...
    def _get_workday_lines(self):
        # self.ensure_one()
        for att_sheet in self:
            contract = att_sheet.contract_id.id
            work_enty_obj = self.env['hr.work.entry.type']
            work_entry_types = att_sheet.contract_id.structure_type_id.default_struct_id.unpaid_work_entry_type_ids
            if not work_entry_types:
                raise ValidationError(_(
                    "Please Add Unpaid Work Entry To %s Salary Structure" % att_sheet.contract_id.name))
...
class AttendanceSheetLine(models.Model):
    ...
    employee_id = fields.Many2one(related='att_sheet_id.employee_id',
                                  string='Employee')
    average_hour= fields.Float(string="Average Hour per Day",related='att_sheet_id.contract_id.resource_calendar_id.hours_per_day')
