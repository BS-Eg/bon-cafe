odoo.define("wk_account_reports.wk_account_report", function (require) {
    ...
    _makeM2MWidget: function (fieldInfo, fieldName) {
      ...
      return this.model
        ...
        .then(function (recordID) {
          self.widgets[fieldName] = new RelationalFields.FieldMany2ManyTags(
            self,
            fieldName,
  ...
  accountReportsWidget.include({
    render_searchview_buttons: function () {
      ...
      var $datetimepickers = this.$searchview_buttons.find(
        ".js_account_reports_datetimepicker"
      );
      var options = {
      ...
      _.each(self.report_options, function (k) {
        if (k !== null && k.filter !== undefined) {
          self.$searchview_buttons
            .find('[data-filter="' + k.filter + '"]')
            .addClass("selected");
        }
      });
      _.each(
        this.$searchview_buttons.find(".js_account_report_bool_filter"),
        function (k) {
          $(k).toggleClass(
            "selected",
      ...
      _.each(
        this.$searchview_buttons.find(".js_account_report_choice_filter"),
        function (k) {
          $(k).toggleClass(
            "selected",
      ...
      $(
        ".js_account_report_group_choice_filter",
        this.$searchview_buttons
      ).each(function (i, el) {
        var $el = $(el);
        ...
        $el.toggleClass(
          "selected",
          _.every(self.report_options[$el.data("filter")], function (member) {
      ...
      _.each(
        this.$searchview_buttons.find(".js_account_reports_one_choice_filter"),
        function (k) {
          $(k).toggleClass(
            "selected",
            "" + self.report_options[$(k).data("filter")] ===
      ...
      // click events
      this.$searchview_buttons
        .find(".js_account_report_date_filter")
        .click(function (event) {
          self.report_options.date.filter = $(this).data("filter");
          var error = false;
          if ($(this).data("filter") === "custom") {
            var date_from = self.$searchview_buttons.find(
              '.o_datepicker_input[name="date_from"]'
            );
            var date_to = self.$searchview_buttons.find(
              '.o_datepicker_input[name="date_to"]'
            );
            if (date_from.length > 0) {
              error = date_from.val() === "" || date_to.val() === "";
              self.report_options.date.date_from = field_utils.parse.date(
                date_from.val()
              );
              self.report_options.date.date_to = field_utils.parse.date(
                date_to.val()
              );
            } else {
              error = date_to.val() === "";
      ...
      this.$searchview_buttons
        .find(".js_account_report_bool_filter")
        .click(function (event) {
          var option_value = $(this).data("filter");
          self.report_options[option_value] =
            !self.report_options[option_value];
      ...
      $(
        ...
        _.each(self.report_options[option_value], function (el) {
          ...
          el.selected =
            !is_selected && option_member_ids.indexOf(Number(el.id)) > -1;
        });
        self.reload();
      });
      this.$searchview_buttons
        .find(".js_account_report_choice_filter")
        .click(function (event) {
          var option_value = $(this).data("filter");
      ...
      var rate_handler = function (event) {
        ...
        if (option_value == "current_currency") {
          ...
          _.each(
            $("input.js_account_report_custom_currency_input"),
            function (input) {
      ...
      $(document).on(
        "click",
        ".js_account_report_custom_currency",
        rate_handler
      );
      this.$searchview_buttons
        .find(".js_account_report_custom_currency")
        .click(rate_handler);
      this.$searchview_buttons
        .find(".js_account_reports_one_choice_filter")
        .click(function (event) {
          self.report_options[$(this).data("filter")] = $(this).data("id");
          self.reload();
        });
      this.$searchview_buttons
        .find(".js_account_report_date_cmp_filter")
        .click(function (event) {
          self.report_options.comparison.filter = $(this).data("filter");
          var error = false;
          var number_period = $(this)
            .parent()
            .find('input[name="periods_number"]');
          self.report_options.comparison.number_period =
          ...
          if ($(this).data("filter") === "custom") {
            ...
            if (date_from.length > 0) {
              error = date_from.val() === "" || date_to.val() === "";
              self.report_options.comparison.date_from = field_utils.parse.date(
                date_from.val()
              );
              self.report_options.comparison.date_to = field_utils.parse.date(
                date_to.val()
              );
            } else {
              error = date_to.val() === "";
      ...
      if (this.report_options.partner) {
        if (!this.M2MFiltersPartner) {
          ...
          if (!_.isEmpty(fields)) {
            ...
            this.M2MFiltersPartner.appendTo(
              this.$searchview_buttons.find(".js_account_partner_m2m")
            );
          }
        } else {
          this.$searchview_buttons
            .find(".js_account_partner_m2m")
            .append(this.M2MFiltersPartner.$el);
        }
      }
      // branch
      ...
      if (this.report_options.analytic) {
        if (!this.M2MFilters) {
          ...
          if (!_.isEmpty(fields)) {
            ...
            this.M2MFilters.appendTo(
              this.$searchview_buttons.find(".js_account_analytic_m2m")
              );
            }
          } else {
          this.$searchview_buttons
            .find(".js_account_analytic_m2m")
            .append(this.M2MFilters.$el);
        }
      }
    },
  });
