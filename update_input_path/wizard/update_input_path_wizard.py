...
class UpdateInputPathWizard(models.Model):
    ...
    def action_create_update_input(self):
        ...
        for input in other_input:
            ...
            fields_exit = self.env['ir.model.fields'].sudo().search([('name','=',f_name),('model_id','=',self.env['ir.model'].search([('model','=','update.input.path')]).id,)])
            if fields_exit.id == False :
                    self.env['ir.model.fields'].sudo().create({'name':'x_'+str(input.id),
                                                               'field_description':input.name,
                                                               'model_id':self.env['ir.model'].search([('model','=','update.input.path')]).id,
                                                               'ttype':'float',
                                                               'relation': False,
                                                               'required': False,
                                                               'store': True,
        ...
        for work_days in work_entry:
            ...
            fields_exit_days= self.env['ir.model.fields'].sudo().search([('name','=',f_name_days),('model_id','=',self.env['ir.model'].search([('model','=','update.input.path')]).id,)])
            f_name_hours='x_'+str(work_days.id)+'_hours'
            fields_exit_hours= self.env['ir.model.fields'].sudo().search([('name','=',f_name_hours),('model_id','=',self.env['ir.model'].search([('model','=','update.input.path')]).id,)])
            inherit_id = self.env.ref('update_input_path.update_input_path_view_tree_new')
            if fields_exit_days.id == False :
                    f=self.env['ir.model.fields'].sudo().create({'name':'x_'+str(work_days.id)+'_days',
            ...
            if fields_exit_hours.id == False:
                    x=self.env['ir.model.fields'].sudo().create(
                    ...
                    view=self.env['ir.ui.view'].sudo().create({'name': 'worked_hours',
                                                          'type': 'tree',
                                                          'model': 'update.input.path',
                                                          'mode': 'extension',
                                                          'inherit_id': inherit_id.id,
