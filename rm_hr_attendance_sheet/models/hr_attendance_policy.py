...
class HrAttendancePolicy(models.Model):
    ...
    def get_late(self,rate,period, cnt):
        self.ensure_one()
        res = period
        flag = False
        if self:
            if self.late_rule_id:
                ...
                for line in time_ids:
                    if cnt <= 5 and cnt == int(line.counter) and  period >= line.time and period != 0.0:
                        flag = True
                        if line.type == 'rate':
                            res = line.rate * rate
                        elif line.type == 'fix':
                        ...
                        # break
                if len(time_ids) > 0 and cnt > 5  and  period >= time_ids[0].time and period != 0.0:
                        flag = True
                        if time_ids[0].type == 'rate':
                            res = time_ids[0].rate * rate
                        elif time_ids[0].type == 'fix':
                            res = time_ids[0].amount
    ...
    def get_diff(self,rate,period,cnt):
        self.ensure_one()
        res = period
        flag = False
        if self:
