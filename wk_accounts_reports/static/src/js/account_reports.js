odoo.define(
  "account_reports_ent_additional.account_report",
  function (require) {
    "use strict";
    ...
    // var M2MBranchFilters = require("account_reports.M2MBranchFilters");
    ...
    var M2MFilters = Widget.extend(StandaloneFieldManagerMixin, {
        ...
        */
        start: function () {
            var self = this;
            var $content = $(
                QWeb.render("m2mWidgetTable", { fields: this.fields })
            );
    ...
    accountReportsWidget.include({
        custom_events: {
            value_changed: function (ev) {
                var self = this;
                var current_ev = ev;
                self.report_options.branch_ids = (ev.data.branch) ? ev.data.branch : self.report_options.branch_ids;
                self.report_options.partner_ids = (ev.data.partner_ids) ? ev.data.partner_ids : self.report_options.partner_ids;
                self.report_options.partner_categories = (ev.data.partner_categories) ? ev.data.partner_categories : self.report_options.partner_categories;
                self.report_options.analytic_accounts = (ev.data.analytic_accounts) ? ev.data.analytic_accounts : self.report_options.analytic_accounts;
                self.report_options.analytic_tags = (ev.data.analytic_tags) ? ev.data.analytic_tags : self.report_options.analytic_tags;
                return self.reload().then(function () {
                    if (current_ev.data.partner_ids) {
                        self.$searchview_buttons.find(".account_partner_filter").click();
                    }
                    if (current_ev.data.analytic_accounts) {
        ...
        render_searchview_buttons: function () {
            ...
            //             this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_branch_m2m'));
            //         }
            //     } else {
            //         this.$searchview_buttons.find('.js_account_branch_m2m').append(this.M2MFilters.$el);
            //     }
            // }
        ...
        _.each(this.$searchview_buttons.find(".js_account_report_choice_filter"), function (k) {
            $(k).toggleClass(
                ...
                return (
                    "" + el.id == "" + $(k).data("id") && el.selected === true
                );
                }).length > 0
            );
        });
        ...
        this.$searchview_buttons.find(".js_account_report_date_filter").click(function (event) {
            ...
            if (this.report_options.partner) {
                ...
                if (!_.isEmpty(fields)) {
                    ...
                    this.M2MPartnerFilters.appendTo(
            ...
            //                 this.$searchview_buttons.find(".js_account_branch_m2m")
            //             );
            //         }
            //     } 
            //     else {
            ...
            // }
                ...
                if (this.report_options.analytic) {
                    ...
                    if (!_.isEmpty(fields)) {
                        this.M2MAlnalyticFilters = new M2MFilters(this, fields);
                        this.M2MAlnalyticFilters.appendTo(
